...
def test_extract_citations_from_result_with_web_search_urls():
    ...
    citations = OpenAIScientificValidator._extract_citations_from_result(result)
    assert citations == ["https://example.org/a", "https://example.org/b"]


def test_extract_citations_from_openai_web_search_sources_payload():
    response = ModelResponse(
        parts=[
            BuiltinToolReturnPart(
                tool_name="web_search",
                ...
                content={
                    "status": "completed",
                    "sources": [
                        {"title": "Source 1", "url": "https://example.org/source-1"},
                        {"title": "Source 2", "url": "https://example.org/source-2"},
                    ],
                },
            ),
        ]
...
def test_process_returns_extracted_citations():
    ...
    validator = OpenAIScientificValidator.__new__(OpenAIScientificValidator)
